...
let ePos = vec2.create();
let eDir = vec2.create();
let dummy = vec2.create();

class Collider {
	static raycast(pos, dir, lines) {
		...
		for (var i=0; i<lines.length; i++) {
			...
			if (newT>0 && newT<t) {
				...
				if (Collider.checkPointInLine(pt, l)) {
					t = newT;
					colPlane = l;
					colPoint = pt; //result!
				}
	...
	static sweepEllipse(pos, dir, lines, eDimensions) {
		var t=1;
		...
		for (var i=0; i<lines.length; i++) {
			var l = Collider.modLine(lines[i], dim);
			...
			if (!(t0>1 || t1<0)) { //we intersect with the line at some point during the time!
				...
				if (Collider.checkPointInLine(pt, l) && newT < t) {
					t = newT;
					colPlane = lines[i];
					colPoint = pt; //result!
					edge = false;
				...
				for (var j=1; j<=2; j++) {
					...
					var root = Collider.getSmallestRoot(vec2.dot(dir, dir), 2*vec2.dot(dir, vert), vec2.dot(vert, vert)-1, t);
					if (root != null) {
						t = root;
						colPlane = lines[i];
	...
	static getSmallestRoot(a, b, c, upperLimit) {
		var det = (b*b) - 4*(a*c);
	...
	static checkPointInLine(pt, line) {
